...
def get_anchors(centroids, width_in_cfg_file, height_in_cfg_file):
    ...
    widths = anchors[:, 0]
    # we want a descending order anchors
...
def kmeans(X, centroids, eps, anchor_file, width_in_cfg_file,
           height_in_cfg_file):
...
def generate_anchor(label_csv, width_in_cfg_file, height_in_cfg_file,
    ...
    if num_clusters == 0:
        for num_clusters in range(1, 11):  # we make 1 through 10 clusters
            ...
            indices = [random.randrange(
                    annotation_dims.shape[0]) for i in range(num_clusters)]
