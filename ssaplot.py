...
def scatter(x, y, df, c=None):
    """Makes a scatter plot with labels and a colorbar as needed.

    Arguments:
        x (str): column name of the x-axis variable in df

        y (str): column name of the y-axis variable in df

        c (str): Default = None. Column name of the color axis variable name in df.

        df (pd.DataFrame): Input dataframe containing the data to be plotted.

    Returns:
        ax (plt.ax object): axis of the scatter plot
    """
    ...
    if c==None:
        ...
        fig, ax = plt.subplots()
        plt.scatter(x=x, y=y, c=c, cmap='plasma', data=df)
        ax.set_xlabel(x)
        ax.set_ylabel(y)
        cbar = plt.colorbar()
        cbar.set_label(c)
